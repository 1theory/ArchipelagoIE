...
from .Types import ItemData, ChapterType, InazumaItem, chapter_type_to_name
...
def create_itempool(world: "InazumaWorld") -> List[Item]:
    ...
    # This is a good place to grab anything you need from option
...
def create_item(world: "InazumaWorld", name: str) -> Item:
    data = item_table[name]
    return InazumaItem(name, data.classification, data.ap_code, world.player)

# Another generic function. For creating a bunch of items at once!
def create_multiple_items(world: "InazumaWorld", name: str, count: int,
                          item_type: ItemClassification = ItemClassification.progression) -> List[Item]:
    data = item_table[name]
    itemlist: List[Item] = []

    for i in range(count):
        itemlist += [InazumaItem(name, item_type, data.ap_code, world.player)]

    return itemlist

# Finally, where junk items are created
def create_junk_items(world: "InazumaWorld", count: int) -> List[Item]:
    ...
    for name in item_table.keys():
        ...
        # It also grabs the trap weights from the options page
            ...
            if name == "Ray Dark Trap":
                trap_list[name] = world.options.RayDarkTrapWeight.value
...
inazuma_items = {
    # Progression items
    "Old Pin Badge": ItemData(96, ItemClassification.progression),
    "Gym Key": ItemData(74, ItemClassification.progression),

    # Useful items
    "Heaven's Time": ItemData(14081, ItemClassification.useful),
    "Triangle Z": ItemData(376, ItemClassification.useful),
    "Prestige Gloves": ItemData(221, ItemClassification.useful),
...
junk_items = {
    ...
    # Traps
    "Ray Dark Trap": ItemData(20050013, ItemClassification.trap, 0)
}
